...
class DAQGUI:
    ...
    def _setupPlots(self):
        ...
        self.curve_vel_time_delta = plt_vel.plot(pen=pg.mkPen("#d62728", width=3), name="Time delta")
        self.curve_vel_time_delta.setZValue(10)
        ...
        plt_squared = self.win.addPlot(row=2, col=1, title="Joule (U=R, V=G, W=B, Total=K)")
        self.curve_Ju = plt_squared.plot(pen=pg.mkPen("#ff4b00", width=3))
        self.curve_Jv = plt_squared.plot(pen=pg.mkPen("#03af7a", width=3))
        self.curve_Jw = plt_squared.plot(pen=pg.mkPen("#005aff", width=3))
        self.curve_Jtot = plt_squared.plot(pen=pg.mkPen("#000000", width=3))
        plt_squared.setLabel("left", "Joule [W]")
    ...
    def _initBuffers(self):
        ...
        self.y_Ju = np.empty(0, dtype=np.float32)
        self.y_Jv = np.empty(0, dtype=np.float32)
        self.y_Jw = np.empty(0, dtype=np.float32)
        self.y_Jtot = np.empty(0, dtype=np.float32)
    ...
    def _refresh(self):
        ...
        try:
            while True:
                ...
                if time_delta_times.size:
                    ...
                    keep_time_delta = self.xs_vel_time_delta >= t_end - time_delta_history_sec
                    self.xs_vel_time_delta = self.xs_vel_time_delta[keep_time_delta]
                    self.y_vel_time_delta = self.y_vel_time_delta[keep_time_delta]
                ...
                self.y_Ju = np.append(self.y_Ju, _Ju)[-self.cfg.dependent.count_history:]
                self.y_Jv = np.append(self.y_Jv, _Jv)[-self.cfg.dependent.count_history:]
                self.y_Jw = np.append(self.y_Jw, _Jw)[-self.cfg.dependent.count_history:]
                self.y_Jtot = np.append(self.y_Jtot, _Jtot)[-self.cfg.dependent.count_history:]
        ...
        #if self.xs_vel.size:
        #    print(self.xs_vel[-1], " " ,self.xs_pow[-1], " ", self.xs_squared[-1])
        #if self.xs_vel.size:
        ...
        self.curve_vel_time_delta.setData(self.xs_vel_time_delta, self.y_vel_time_delta)
        self.curve_vel_ref.setData(self.xr_vel, self.yr_vel)
        # Power
        self.curve_pow_tot.setData(self.xs_pow, self.y_pow_tot)
        # Squared current
        self.curve_Ju.setData(self.xs_squared, self.y_Ju)
        self.curve_Jv.setData(self.xs_squared, self.y_Jv)
        self.curve_Jw.setData(self.xs_squared, self.y_Jw)
        self.curve_Jtot.setData(self.xs_squared, self.y_Jtot)
