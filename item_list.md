...
1. A相信号立上がりからの時間差計測と速度計測
  ...
  - サンプリングレートは5~10 samplings/sとすること (サンプリング数に対応する時間0.1~0.2 sをDurationと定義する)。
  - A相信号のサンプリングレートは1 rps 回転時の4逓倍信号を仮定して1/2000 ~ 0.5 msとすること。
  - AD変換後のデータ点間隔は0.05 ~ 0.1 msとすること。
...
1. AおよびB相信号立上がりからのn逓倍パルスカウントと速度計測
  - パルスカウントの機能を持つこと。
  - サンプリングレートは5~10 samplings/sとすること（１と一致すること）。
  - 1 rps 回転時の4逓倍信号を仮定して1/2000 ~ 0.5 msのパルス信号をカウントすること。
...
1. データログ
  - ソフトウェア処理後の時間差と速度、パルスカウントと速度を保存すること。
...
#### エンコーダー部
1. AD変換部
  - 変換器: National Instruments USB-9215
...
3. グラフ
  ...
  - 方式: PykstでのAPI呼び出しによる描画 (Tentative)
